...
import './RegistroAhorros.css'; // Importa el CSS de estilos
import { useUser } from '../../contexts/UserContext';

const RegistrosAhorros = () => {
  ...
  // Función para crear registros de ahorros de la fecha actual
  const crearRegistroAhorros = async () => {
    const confirmacion = window.confirm('¿Estás seguro de que quieres crear un nuevo registro de Ahorros con la fecha actual?');
    if (confirmacion) {
      try {
        const response = await axios.post('https://localhost:7190/api/RegistroPagoAhorroController/crearRegistroPagoAhorro', {});
        if (response.status === 200) {
          // Recargar los registros después de crear uno nuevo
          loadRegistros();
        } else {
          console.error('Error al crear el registro de ahorros');
        }
      } catch (error) {
  ...
  const loadRegistros = async () => {
    try {
      const response = await axios.get('https://localhost:7190/api/RegistroPagoAhorroController/mostrarRegistroPagoAhorros');
      ...
      const registrosConPrueba = [...registrosFormateados, ...datosPrueba];
 ...
 if(!user) return null;
  ...
  return (
    <div className="gestion-convenios-container d-flex flex-column align-items-center">
      ...
      {/* Texto para crear registros de ahorros de la fecha actual */}
      <p className="mb-4">¿Quieres crear un registro de Ahorros con la fecha actual?</p>

      {/* Botón para crear registros de ahorros de la fecha actual */}
      <button className="btn btn-success btn-lg btn-add2 mb-4" onClick={crearRegistroAhorros}>
        Exportar
      </button>
...
export default RegistrosAhorros;
